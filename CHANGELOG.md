...
- Merge `:has-text()` rules across every selector separator, not just `##`. An exception or procedural group (`#@#`, `#?#`, `#@?#`) with the same domains and base selector now folds into one regex the way a hiding group already did; each separator groups on its own, so a hiding rule and an exception are never combined.
