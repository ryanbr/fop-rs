...
- Add `--remove-non-domain-on-add`, which deletes an added line that is a bare word rather than a domain. A line such as `isCookiesAccepted` -- one argument of a scriptlet, pasted adrift -- is a legal rule: it blocks any URL containing that text, so nothing flagged it, and sorting filed it quietly among the rules. It is also indistinguishable from a deliberate substring rule, which is why the default is unchanged and this is opt-in. Two things keep it narrow. It judges additions only, as all the rule checks do, so the 911 bare-word rules already committed across easylist, uAssets, AdguardFilters and test-lists are never candidates. And a leading or trailing `-` or `_` exempts a line, which is how a substring rule is written: 900 of those 911 carry one. The other 11 are real rules too -- `fingerprintjs`, `728x90px`, `page_view_count` -- so the flag is for a list whose author knows they do not write that shape. It removes only what it flags; every other defect still needs `--remove-bad-rules`, and the existing guard that holds back a line which may carry a committed rule merged into it still applies.
