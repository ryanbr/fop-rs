...
#[test]
fn test_which_hash_lines_a_hosts_file_may_carry() {
    // `#` opens a comment in a hosts file and a rule in a filter list. Reading
    // only `# ` and a run of `#` as comments left the banner styles hosts
    // files are actually written in -- `#=====`, `#Title: ...` -- looking like
    // rules, which disqualified the file; its comments were then whitespace-
    // stripped, sorted among the entries, and stopped ending their sections.
    //
    // Widening it has one hard limit: a line that could begin a rule must go
    // on disqualifying, or a list of generic rules would be read as a file
    // with no rules at all and a stray entry would carry it -- and every rule
    // in it would be commented out.
    use crate::fop_sort::looks_like_hosts_file;
    let with = |first: &str| {
        looks_like_hosts_file(
            format!("{}\n0.0.0.0 zulu.example\n127.0.0.1 alpha.example\n", first).as_bytes(),
        )
    };
    // Comments a hosts file carries.
    for line in ["#", "####", "# a heading", "#=====", "#-----", "#Title: my hosts",
                 "#Generated by a script", "#!/bin/sh"] {
        assert!(with(line), "a hosts file was not recognised because of {:?}", line);
    }
    // Anything that could be a rule.
    for line in ["##.ad", "###banner", "#@#.ad", "#?#div:has(a)", "#$#body { color: red; }",
                 "#%#//scriptlet('x')", "#ad-banner", "#tracker", "##### AdAway #####"] {
        assert!(!with(line), "{:?} did not disqualify the file", line);
    }
    // A file with no rules at all is not a hosts file either.
    assert!(!looks_like_hosts_file(b"# only\n#=====\n"));
}

#[test]
fn test_the_checks_judge_a_hosts_comment_as_the_sorter_writes_it() {
    // `tidy_rule` mirrors the sorter so the checks judge a line in the form it
    // will be written. A file read as a hosts file keeps its comments as
    // written, but `tidy_rule` sees one line at a time and answered for the
    // filter-list reading -- stripping the spaces out of `#Title: my hosts`.
    // The checks would then have judged, and `--remove-bad-rules` deleted on,
    // a form that is not in the file.
    let repo = ScratchRepo::new("hostscomment");
    repo.write("hosts.txt", "# seed\n0.0.0.0 seed.example\n");
    repo.git(&["add", "."]);
    repo.git(&["commit", "-q", "-m", "base"]);
    repo.write(
        "hosts.txt",
    ...
    for (add, as_sorted) in additions.iter().zip(&tidied) {
        if add.content.starts_with('#') {
            assert_eq!(
                &add.content, as_sorted,
                "a hosts-file comment was judged in a form the sorter does not write"
            );
        }
    }
}
