...
| `--check-rules-on-add` | Check git additions for rules that cannot work (empty selector, unbalanced brackets, malformed domain list, unknown or empty option, bare hostname), and prompt before committing; with `--ci` it audits the committed diff and exits 1 on a defect |
